...
public class MailService {
    ...
    public void sendCode(String email) {
        ...
        // 사용자가 입력한 이메일의 앞뒤 공백 제거
        email = email.trim();

        // 사용자가 입력한 이메일을 Entity에 넣음
    ...
    public void verifyCode(EmailVerifyRequestDto dto) {
        ...
        // 사용자가 입력한 이메일 앞뒤 공백 제거
        String email = dto.getEmail().trim();

        // 사용자가 입력한 인증번호 앞뒤 공백 제거
        String code = dto.getCode().trim();

        EmailVerificationEntity entity = emailVerificationRepository.findById(email)
